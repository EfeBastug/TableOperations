...
public class Solution {
    ...
    private List<String> reverseCols;
    ...
    public Solution(Squares question, Squares answer) {
        ...
        this.rotations = List.of("90"); // Combined list for all rotations

        this.reverseCols = generateReverseColumns(size);
    ...
    private List<String> generateReverseColumns(int size) {
        StringBuilder sb = new StringBuilder();
        List<String> revertColumns = new ArrayList<>();
        for (char col = 'A'; col < 'A' + size; col++) {
            sb.append(col).append(col);
            revertColumns.add(sb.toString());
            sb.setLength(0);
        }
        return revertColumns;
    }
    ...
    public void revertColumn(int firstColumnIndex, int secondColumnIndex, ArrayList<ArrayList<Integer>> board) {
        if (firstColumnIndex != secondColumnIndex) {
            System.out.println("Column reverting gone wrong.");
            return;
        }
        int top = 0;
        int bottom = board.size() - 1;
        // Swap elements from top to bottom of the specified column
        while (top < bottom) {
            // Swap elements in the column
            int temp = board.get(top).get(firstColumnIndex);
            board.get(top).set(firstColumnIndex, board.get(bottom).get(firstColumnIndex));
            board.get(bottom).set(firstColumnIndex, temp);
    ...
    private int generateOperationSequences(List<String> currentOps, int depth) {
        ...
        boolean hasReverseCols = currentOps.stream().anyMatch(reverseCols::contains);

        List<String> allOps = new ArrayList<>(rowSwaps);
        allOps.addAll(colShifts);
        allOps.addAll(rotations);
        allOps.addAll(reverseCols);
        ...
        for (String operation : allOps) {
            ...
            boolean isReverseCol = reverseCols.contains(operation);

            if ((isRowSwap && hasRowSwap) || (isColSwap && hasColShifts) || (isRotations && hasRotations) || (isReverseCol && hasReverseCols)) {
    ...
    private void applyOperation(String operation, ArrayList<ArrayList<Integer>> board)  {
        if (rotations.contains(operation)) {
            ...
            if (isValidRowSwap(operation)) {
                int baseRowChar = 'A' + size; // 'D' for 3x3, 'E' for 4x4, 'F' for 5x5
        ...
        else if (reverseCols.contains(operation)) {
            int firstColumnIndex = operation.charAt(0) - 'A';
            int secondColumnIndex = operation.charAt(0) - 'A';
            if (isValidIndex(firstColumnIndex) && isValidIndex(secondColumnIndex)) {
                revertColumn(firstColumnIndex, secondColumnIndex, board);
            }
        }
    }

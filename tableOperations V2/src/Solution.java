...
public class Solution {
    ...
    public Solution(Squares question, Squares answer) {
        ...
        this.rowShifts = generateRowShifts(size);
    ...
    //Helper method to generate row shifts based on board size
    private List<String> generateRowShifts(int size) {
        List<String> rowShifts = new ArrayList<>();
        char rowChar;
        switch (size) {
        ...
        for (char row = rowChar; row < rowChar + size; row++) {
            rowShifts.add(String.valueOf(row));
        }
        return rowShifts;
    }
    ...
    private void shiftRow(int rowIndex, ArrayList<ArrayList<Integer>> board) {
    ...
    private int generateOperationSequences(List<String> currentOps, int depth) {
        ...
        boolean hasRowShifts = currentOps.stream().anyMatch(colShifts::contains);
        ...
        for (String operation : allOps) {
            ...
            boolean isRowShift = rowShifts.contains(operation);
            boolean isColShift = colShifts.contains(operation);
            boolean isRotations = rotations.contains(operation);

            if ((isRowSwap && hasRowSwap) || (isRowShift && hasRowShifts) && (isColShift && hasColShifts) || (isRotations && hasRotations)) {
                continue;
            }
    ...
    private void applyOperation(String operation, ArrayList<ArrayList<Integer>> board) {
        ...
        else if (rowShifts.contains(operation)) {
            int firstRowIndex = operation.charAt(0) - 'A' - size;
            if (isValidIndex(firstRowIndex)) {
                shiftRow(firstRowIndex, board);
            }
        }
